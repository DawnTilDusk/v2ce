    ...
    if voxel_grid.ndim == 5:
        ...
        if keep_polarity:
            ...
            if P >= 2:
                ...
                pos_flat = pos_flat[pos_flat > 0]
                neg_flat = neg.flatten()
                neg_flat = neg_flat[neg_flat > 0]
                if pos_flat.size == 0 and neg_flat.size == 0:
                    frame = np.zeros((H, W, 3), dtype=np.uint8)
                    return frame
                pos_ub = min(np.percentile(pos_flat, upper_bound_percentile), ceil) if pos_flat.size > 0 else 1.0
                neg_ub = min(np.percentile(neg_flat, upper_bound_percentile), ceil) if neg_flat.size > 0 else 1.0
                pos = np.clip(pos, 0, pos_ub) / (pos_ub if pos_ub > 0 else 1.0)
                neg = np.clip(neg, 0, neg_ub) / (neg_ub if neg_ub > 0 else 1.0)
